...
| `conditionalFormats` (`backgroundScale` variant) on `pivot-table` | Probe workbook `bfd62356-fde3-4fcb-968a-a562bc7d3d30` ("Healthcare" folder) — 2026-08-13 | **POST-verified from scratch.** A 3-column pivot (`rowsBy`/`columnsBy`/`values`) with `backgroundScale` (3-color `scheme`) on its value column POSTed successfully (200, `audit-workbook-schema.sh` clean), `GET /v2/workbooks/{id}/spec` returned 200 with the format present (no `service_error`), and `verify-workbook.sh` compiled it clean. This is a direct retest of the "Unverified" table's `backgroundScale`/4-kinds row below, narrowed to `pivot-table` specifically — see that row for what's still open. |
